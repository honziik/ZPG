    ...
    : vertexCount(vertexCount), texture(nullptr), hasIBO(false), indexCount(0), IBO(0), indicesCount(0)
{
    ...
    // Set position attribute (always present)
...
Models::Models(const std::string& fileName, const std::string& textureName)
{
    loadFromFile(fileName, textureName);
}
struct Vertex
{
    float Position[3];
...
bool Models::loadFromFile(const std::string& fileName, const std::string& textureName)
{
...
void Models::setTexture(Texture* tex) {
    this->texture = tex;
}
